let categorias = function (){
    let initDataTable = function () {
        let oTable = $("#categorias-datatable").DataTable({
            ...
            serverSide: true,
            ajax: {
                url: $urlBase + "/blog/categorias/get",
                dataType: "json"
            },
            pageLength: 50,
            columns: [{
        ...
        $("#categorias-datatable tbody").on("click", "#btnEditar, #btnEliminar, #btnVer", function (){
            
            let oTable = $("#categorias-datatable").DataTable();
            var data = oTable.row($(this).parents('tr')).data();
            
            if(this.id == "btnEditar"){
                location.href = $urlBase + "/blog/categorias/edicion/" + data.id;
            }
            ...
            if(this.id == "btnEliminar"){
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            method: 'GET',
                            url: $urlBase + "/blog/categorias/delete/" + data.id,
        ...
        $("#btnNuevo").on("click", function (e){
            location.href = $urlBase + "/blog/categorias/alta";
        });
    }
    return {
        init: function () {
